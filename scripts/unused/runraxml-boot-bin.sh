...
if [ "$donebs" == "" ]; then
 ...
 if [ $CPUS -gt 1 ]; then
  $DIR/raxmlHPC-PTHREADS -m $model -T $CPUS -n best -s ../$in.phylip -M -q ../$in.part $s -N $bestN $startingtree &> ../logs/best_std.errout.$in
 else
  $DIR/raxmlHPC -m $model -n best -s ../$in.phylip $s -M -q ../$in.part -N $bestN $startingtree &> ../logs/best_std.errout.$in
 fi
fi
...
if [ "$donebs" == "" ]; then
  ...
  if [ $crep -gt 0 ]; then
   if [ $CPUS -gt 1 ]; then
      $DIR/raxmlHPC-PTHREADS -m $model -n ml -s ../$in.phylip -M -q ../$in.part -N $crep $boot -T $CPUS  $s &> ../logs/ml_std.errout.$in
   else
      $DIR/raxmlHPC -m $model -n ml -s ../$in.phylip -M -q ../$in.part -N $crep $boot $s &> ../logs/ml_std.errout.$in
   fi
  fi
fi
